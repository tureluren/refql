...
interface Prop<As extends string = any, Output = any, Params = any, IsOmitted extends boolean = any, HasDefault extends boolean = any> extends RQLNode, PropType<As> {
  ...
  asc(): Prop<As, Output, Params, IsOmitted, HasDefault>;
  desc: Prop<As, Output, Params, IsOmitted, HasDefault>["asc"];
  operations: Operation<Params>[];
  omit(): Prop<As, Output, Params, true, HasDefault>;
  hasDefault(): Prop<As, Output, Params, IsOmitted, true>;
}
...
function Prop<As extends string, Output = any, Params = any>(as: As, col?: string) {
  let prop: Prop<As, Output, Params> = Object.create (prototype);
